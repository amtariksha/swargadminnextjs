/**
 * GET /api/agent-tools/lms/read-health-score?customerId=<uuid>
 *
 * Returns the latest health-score row for one customer, or null if never
 * computed. Used by: lms-insights, lms-customer-support-assist.
 */
...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        return ok({ customerId: parsed.data.customerId, health: data ?? null });
    } catch (err) {
        return fail(err);
    }
}
